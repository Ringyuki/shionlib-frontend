...
import { FolderIcon, FolderOpenIcon, FileText } from 'lucide-react'
...
} from '@/components/shionui/animated/libs/primitives/base/files'
...
function Files({ className, children, ...props }: FilesProps) {
  return (
    <FilesPrimitive className={cn('p-2 w-full', className)} {...props}>
      <FilesHighlightPrimitive className="bg-primary/10 rounded-lg">
        {children}
      ...
      <FolderTriggerPrimitive className="w-full text-start">
        <FolderHighlightPrimitive>
          <FolderPrimitive className="flex items-center justify-between gap-2 p-2">
            ...
            >
              <FolderIconPrimitive
                closeIcon={<FolderIcon className="size-4.5 text-orange-400" />}
                openIcon={<FolderOpenIcon className="size-4.5 text-orange-400" />}
              />
...
function FileItem({
  ...
  return (
    <FileHighlightPrimitive className={cn('hover:text-primary transition-colors', className)}>
      <FilePrimitive
        className={cn(
          'flex items-center justify-between gap-2 p-2 duration-300',
          gitStatus === 'untracked' && 'text-green-400',
          gitStatus === 'modified' && 'text-amber-400',
          gitStatus === 'deleted' && 'text-red-400',
        )}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <FileLabelPrimitive className={cn('text-sm', className)}>{children}</FileLabelPrimitive>
        </div>
